...
Regaring $$I_c$$ (from now on we drop the subscript $$_c$$) the main equation is
...
$$
\frac{\part I}{\part M} = 
\frac{\part I}{\part w}\frac{\part w}{\part \mathcal D} + 
\frac{\part I}{\part w}\frac{\part w}{\part \mathcal z} + 
\frac{\part I}{\part C}
...
Finding $$\part I^i/\part C$$ is direct, and for $$\part I/\part x,y$$ and  $$\part I /\part z$$ the quations are longer:


$$
\begin{align}
\text{Color derivative:} & & \frac{\part I^i}{\part C_j^i} &= w_j^i dC_j^i \\ \\

\text{Probability map derivative:} && \frac{\part I^i}{\part \mathcal D_j^i} &= \sum_{k\not = j} \left(  C_k^i \frac{\part w_k^i}{\part \mathcal D_j^i} \right) + C_j^i\frac{\part w_j^i}{\part \mathcal D_j^i} + C_b\frac{\part w_b^i}{\part \mathcal D_j^i} \\ \\

\text{Depth derivative:} && \frac{\part I^i}{\part z_j^i} &=

\sum_{k\not = j} \left(  C_k^i \frac{\part w_k^i}{\part z_j^i} \right) + 
C_j^i\frac{\part w_j^i}{\part z_j^i} + C_b\frac{\part w_b^i}{\part z_j^i}
\end{align}
$$
...
\frac{\part w_j^i}{\part \mathcal D_j^i} &=\frac{w_j^i}{\mathcal D_j^i}d\mathcal D_j^i - \frac{w_j^iw_j^i}{\mathcal D_j^i}d\mathcal D_j^i \\\\

\frac{\part w_b^i}{\part \mathcal D_j^i} &= -\frac{w_b^iw_j^i}{\mathcal D_j^i}d\mathcal D_j^i
...
All together in $$\part I / \part \mathcal D$$ simplifies the quations to


$$
\begin{align}
\frac{\part I^i}{\part \mathcal D_j^i} &= \left(
...
\frac{\part \mathcal D_j^i}{\part u_{k,l}} &= \mathcal D_j^i(1-\mathcal D_j^i) \left(\frac{1}{\sigma} \frac{\part D(i,j)}{\part u_{k,l}} \right) \\\\

\frac{\part D(i,j)}{\part u_{k,l}} &= 2 \pmb \lambda_s d\pmb\lambda_s 

\hspace{1cm} \text{where} \hspace{1cm} s = \arg \min_i(\pmb \lambda_i)\\

&= 2 (U^{-1}\pmb p_i)_sd(U^{-1}\pmb p_i)_s \\

&= 2 \pmb \lambda_s \left( -U^{-1} \frac{\part U}{\part u_{k,l}} U^{-1} \right)\pmb p_i\\\\

\frac{\part U_j}{\part u_{k,l}} &= \left(\begin{matrix} 
1 & 0 & 0\\ 
0 & 0 & 0\\ 
...
Doing the corresponding math to calculate $$\part U_j/\part u_{k,l}$$ the final result is 


$$
\frac{\part \pmb \lambda_s}{\part u_{k,l}} = -v_{s,k}\sum_tv_{l,t}p_t \hspace{1cm} \text{with} \hspace{1cm} V=U^{-1}=\{v_{m,n}\}
$$
...
$$
\begin{align}
\frac{\part I^i}{\part \mathcal D_j^i} &= \frac {w_j^i}{\mathcal D_j^i} \left( C_j^i - I^i \right)d\mathcal D_j^i \\\\

\frac{\part I^i}{\part \mathcal D_j^i}\frac{\part \mathcal D_j^i}{\part u_{k,l}}&= \frac {w_j^i}{\mathcal D_j^i} \left( C_j^i - I^i \right)\left( \frac{\mathcal D_j^i(1-\mathcal D_j^i) }{\sigma} \frac{\part D(i,j)}{\part u_{k,l}}   \right) \\\\

\frac{\part I^i}{\part \mathcal D_j^i}\frac{\part \mathcal D_j^i}{\part u_{k,l}}&= \frac {w_j^i}{\mathcal D_j^i} \left( I^i - C_j^i  \right)\left( \frac{\mathcal D_j^i(1-\mathcal D_j^i) }{\sigma} \left(2 \pmb \lambda_s v_{s,k}\sum_tv_{l,t}p_t  \right)  \right) \\\\
\end{align}
$$

### Depth derivatives [^2]

The result for $$\part w_j^i/\part z_j^i$$ follows a very similar line of the one for $$\part w_j^i/\part \mathcal D_j^i$$, 


$$
\begin{align}
\frac{\part w_j^i}{\part z_j^i} &= \frac{w_j^i}{\gamma}dz_j^i - \frac{w_j^iw_j^i}{\gamma}d z_j^i \\\\


\frac{\part w_k^i}{\part z_j^i} &= - \frac{w_k^iw_j^i}{\gamma}d z_j^i\\\\


\frac{\part w_b^i}{\part z_j^i} &= -\frac{w_b^iw_j^i}{\gamma}d z_j^i
\end{align}
$$
...
$$
\frac{\part I^i}{\part z_j^i} = \frac {w_j^i}{\gamma} \left( C_j^i - I^i \right)d z_j^i
$$
...
$$
dz_j^i = \frac{\part z_j^i}{\part u_{k,l}} +\frac{\part z_j^i}{\part z_j^{(m)}}
$$
...
$$
\begin{align}
\frac{\part z_j^i}{\part z_{k}^{(m)}} &= \frac{1}{z_\text{far}-z_\text{near}}\left( \frac{-\lambda_j^{(m)}}{\left(z_j^{(m)}\sum_t \lambda_j^{(t)}/z_j^{(t)}\right)^2} \right)\\\\

\frac{\part z_j^i}{\part u_{k,l}} &= \frac{-1}{z_\text{far}-z_\text{near}} \left( \frac{\displaystyle\sum_w\left[\left(z_j^{(w)}\right)^{(-1)}\part \lambda_j^{(w)}/\part u_{k,l}\right]}{\left( \sum_w\lambda_j^{(w)}/z_j^{(w)} \right)^2} \right) \\\\
...
$$
\begin{align}
\frac{\part I^i}{\part z_j^i} &= \frac{w_j^i}{\gamma}(C_j^i- I^i)dz_j^i\\\\


\frac{\part I^i}{\part z_j^i}\frac{\part z_j^i}{\part z_j^{(m)}} &= \frac {w_j^i}{\gamma} \left( I^i -  C_j^i \right)\left(  \frac{\lambda_j^{(m)}}{\left(z_\text{far}-z_\text{near} \right)\left(z_j^{(m)}\sum_t \lambda_j^{(t)}/z_j^{(t)}\right)^2}\right) && \text{with } U_j = \{u_{m,n}\}\\\\

\frac{\part I^i}{\part z_j^i}\frac{\part z_j^i}{\part u_{k,l}} &= \frac {w_j^i}{\gamma} \left( I^i -  C_j^i \right) \left(  \frac{\displaystyle\sum_w\left[\frac{v_{w,k}}{z_j^{(w)}}\sum_tv_{l,t}p_t^i\right]}{\left(z_\text{far}-z_\text{near} \right)\left( \sum_w\lambda_j^{(w)}/z_j^{(w)} \right)^2} \right) &&\text{with } V_j = U_j^{-1} = \{ v_{m,n}\}
\end{align}
$$
...
$$
\frac{\part I^i}{\part C_j^i} = w_j^i
$$
...
$$
\frac{\part I^i_s}{\part \mathcal D_j^i} = \frac{1-I_s^i}{1-\mathcal D_j^i}d\mathcal D_j^i
$$
 In other words we know that $$D$$ depends only on $$\pmb \lambda$$ which only depends on $$u_{k,l}$$. Therefore


$$
\frac{\part I^i_s}{\part \mathcal D_j^i} = \frac{1-I_s^i}{1-\mathcal D_j^i}\frac{\part \mathcal D_j^i}{\part u_{k,l}}
$$
...
$$
\begin{align}
\frac{\part \mathcal L}{\part \hat I_c}\frac{\part \hat I_c}{\part u_{k,l}} &= \mu_c\frac{\part \mathcal L_c}{\part \hat I_c} \frac{\part \hat I_c}{\part u_{k,l}}\\\\
\frac{\part \mathcal L}{\part \hat I_c}\frac{\part \hat I_c}{\part z_k^{(m)}} &= \mu_c\frac{\part \mathcal L_c}{\part \hat I_c} \frac{\part \hat I_c}{\part z_{k}^{(m)}} \\\\
\frac{\part \mathcal L}{\part \hat I_s}\frac{\part \hat I_s}{\part u_{k,l}} &= \frac{\part \mathcal L_s}{\part \hat I_s}\frac{\part \hat I_s}{\part u_{k,l}}\\\\
\frac{\part \mathcal L}{\part \hat I_c}\frac{\part \hat I_c}{\part C_j^i} &= \mu_c \frac{\part \mathcal L_c}{\part \hat I_c}\frac{\part \hat I_c}{\part C_j^i}
\end{align}
$$


$$\part \mathcal L_c/\part I_c$$ and $$\part \mathcal L_s /\part \hat I_s$$  are already calculated for us, thus we only care about calculating the partial derivatives of $$\hat I_c$$  and $$\hat I_s$$. 
...
$$
\begin{align}
\frac{\part \mathcal L}{\part u_{k,l}} &= \frac{\part \mathcal L_s}{\part \hat I_s}\frac{\part \hat I_s}{\part u_{k,l}} + \mu_c\frac{\part \mathcal L_c}{\part \hat I_c} \frac{\part \hat I_c}{\part u_{k,l}} \\\\
\frac{\part \mathcal L}{\part z_k^{(m)}} &= \mu_c\frac{\part \mathcal L_c}{\part \hat I_c} \frac{\part \hat I_c}{\part z_{k}^{(m)}}\\\\
\frac{\part \mathcal L}{\part C_j^i} &= \mu_c \frac{\part \mathcal L_c}{\part \hat I_c}\frac{\part \hat I_c}{\part C_j^i}
\end{align}
$$
...
$$
\begin{align}
\frac{\part I^i_s}{\part u_{k,l}} &= \frac{1-I_s^i}{1-\mathcal D_j^i}\frac{\part \mathcal D_j^i}{\part u_{k,l}}\\\\
...
$$
\begin{align}
\frac{\part I^i}{\part u_{k,l}} &= 
\frac{\part I^i}{\part \mathcal D_j^i}\frac{\part \mathcal D_j^i}{\part u_{k,l}}+ 
\frac{\part I^i}{\part z_j^i}\frac{\part z_j^i}{\part u_{k,l}}\\\\

\frac{\part I^i}{\part \mathcal D_j^i}\frac{\mathcal D_j^i}{\part u_{k,l}} &= 
...
$$
\begin{align}
\frac{\part I^i}{\part z_j^{(m)}} &= \frac{\part I^i}{\part z_j^i}\frac{\part z_j^i}{\part z_j^{(m)}} \\\\
